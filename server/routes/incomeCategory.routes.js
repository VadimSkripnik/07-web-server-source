...
router.patch("/:incomeCategoryId", auth, async (req, res) => {
  try{
      ...
      if( incomeCategoryId) {
          const updatedIncomeCategory = await IncomeCategory.findByIdAndUpdate(incomeCategoryId, req.body, {new: true})
...
router.delete('/:incomeCategoryId', auth, async (req, res) => {
  try {
    const { incomeCategoryId } = req.params
    const removedIncomeCategory = await IncomeCategory.findById(incomeCategoryId)
